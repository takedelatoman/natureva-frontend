...
import * as ImagePicker from 'expo-image-picker'; // <-- Importamos la nueva librería
...
export default function LabValuesScreen({ route, navigation }: any) {
  ...
  // NUEVO ESTADO: Guardamos la ruta de la foto aquí
  ...
  const takePhoto = async () => {
    ...
    const result = await ImagePicker.launchCameraAsync({
      allowsEditing: true,
      quality: 0.8, // Calidad buena pero sin pesar demasiados megas
    });
    if (!result.canceled) {
  ...
  const handleContinue = () => {
    const todosLosDatos = {
      ...
      fotoAdjunta: imageUri // <-- ¡El backend ya recibirá la imagen aquí!
    };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {/* --- SECCIÓN DE FOTO ACTUALIZADA --- */}
        {imageUri ? (
          // Si ya tomó una foto, le mostramos la miniatura y un botón para cambiarla
          ...
          // Si no hay foto, mostramos el botón punteado original
...
const styles = StyleSheet.create({
  ...
  // Nuevos estilos para cuando YA hay foto
  imagePreviewContainer: {
    width: '100%', alignItems: 'center', marginBottom: 20,
  },
  previewImage: {
    width: '100%', height: 200, borderRadius: 15, borderWidth: 1, borderColor: '#CCC', marginBottom: 10,
  },
  changeImageButton: {
    flexDirection: 'row', alignItems: 'center', backgroundColor: '#7EBAE4', 
