...
export default function MentalRiskScreen({ route, navigation }: any) {
  ...
  const handleContinue = async () => {
    // Validar que haya seleccionado al menos un factor o escrito algo
    if (selectedRisks.length === 0 && !otherRisk.trim()) {
      Alert.alert(
        'Aviso', 
        'Por favor, selecciona al menos un factor de riesgo o comparte algo.'
      );
      return;
    }
    ...
    try {
      ...
      const resultado = await mentalHealthService.guardarSaludMental(
        selectedRisks,
        otherRisk.trim()
      );
      
      console.log('✅ Salud mental guardada:', resultado);

      // Combinar todos los datos
      const todosLosDatos = {
        ...datosPrevios,
        saludMental: {
          opcionesSeleccionadas: selectedRisks,
          otroRiesgo: otherRisk.trim()
        },
        saludMentalGuardado: resultado
      };

      console.log('📦 JSON FINAL PARA EL BACKEND:', JSON.stringify(todosLosDatos, null, 2));
      ...
      Alert.alert(
        'Error', 
        error.message || 'No se pudieron guardar los datos. Por favor, intenta de nuevo.'
      );
      console.error('❌ Error al guardar salud mental:', error);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <Text style={styles.subtitle}>
          Las emociones impactan su salud física. ¿Ha experimentado algo de esto últimamente?
        </Text>
        ...
        <TouchableOpacity 
          style={[
            ...
            ((selectedRisks.length === 0 && !otherRisk.trim()) || isLoading) && styles.disabledButton
          ]}
          disabled={selectedRisks.length === 0 && !otherRisk.trim() || isLoading}
