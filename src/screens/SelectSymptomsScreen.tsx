// SelectSymptomsScreen.tsx - COMPLETO CON API
...
export default function SelectSymptomsScreen({ route, navigation }: any) {
  // Recibimos los datos que vienen desde la selección de diagnósticos
  ...
  // ✅ Guardar síntomas en el backend
  const handleContinue = async () => {
    // Validar que haya al menos un síntoma o texto en "otro"
    if (selectedSymptoms.length === 0 && !otherSymptom.trim()) {
      Alert.alert('Atención', 'Selecciona al menos un síntoma o escribe otro');
      return;
    }
    ...
    try {
      ...
      const response = await sintomaService.guardarSintomas(
        selectedSymptoms,
        otherSymptom.trim()
      );

      if (response.success) {
        console.log('✅ Síntomas guardados:', response.data);

        // Juntamos toda la información acumulada
        const todosLosDatos = {
          ...datosPrevios,
          sintomas: selectedSymptoms,
          otrosSintomas: otherSymptom.trim(),
          sintomaId: response.data?.id || null,
        };
        ...
        Alert.alert('Error', response.message || 'Error al guardar síntomas');
      }
    } catch (error) {
        ...
        >
          ...
          <TouchableOpacity
            style={[
              ...
              (selectedSymptoms.length === 0 && !otherSymptom.trim()) && styles.disabledButton
            ]}
            disabled={selectedSymptoms.length === 0 && !otherSymptom.trim() || loading}
...
const styles = StyleSheet.create({
  ...
  chip: {
    ...
    paddingVertical: 18, paddingHorizontal: 10, borderRadius: 15,
