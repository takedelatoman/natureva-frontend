...
export default function HabitsScreen({ route, navigation }: { route: RouteParams; navigation: any }) {
  ...
  const toggleHabit = (habit: string): void => {
  ...
  const handleContinue = async (): Promise<void> => {
    // Validar que haya seleccionado al menos un hábito
    if (selectedHabits.length === 0 && !otherHabit.trim()) {
      Alert.alert(
        'Aviso', 
        'Por favor, selecciona al menos un hábito o describe otro.'
      );
      return;
    }
    ...
    try {
      // Guardar en el backend usando el servicio
      const resultado = await habitsService.guardarHabitos(
        selectedHabits,
        otherHabit.trim()
      );
      
      console.log('✅ Hábitos guardados:', resultado);

      // Combinar con datos previos
      const todosLosDatos = {
        ...datosPrevios,
        habitos: {
          opcionesSeleccionadas: selectedHabits,
          otroHabito: otherHabit.trim()
        },
        habitosGuardados: resultado
      };
      ...
      Alert.alert(
        'Error', 
        error.message || 'No se pudieron guardar los hábitos. Por favor, intenta de nuevo.'
      );
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <Text style={styles.subtitle}>
          Seleccione los hábitos que mejor describen su día a día.
        </Text>
        ...
        <TouchableOpacity 
          style={[
            ...
            ((selectedHabits.length === 0 && !otherHabit.trim()) || isLoading) && styles.disabledButton
          ]}
          disabled={selectedHabits.length === 0 && !otherHabit.trim() || isLoading}
